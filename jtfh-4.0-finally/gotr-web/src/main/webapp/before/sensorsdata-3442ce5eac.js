 ...
 * @modify yuguangcan@gotokeep.com
 * @description 本地化后的神策sdk，用法见https://www.sensorsdata.cn/manual/js_sdk.html，但收集的数据发送到Keep服务器
 */
try{
  ...
  (function(sd) {
    ...
    sd.para.custom_header = sd.para.custom_header || {};
    // 是否需要非cache，等于每次请求文件
    ...
    sd.sendState.getSendCall = function(data, callback) {
      ...
      if(sd.para.server_url.indexOf('?') == -1){
        url = sd.para.server_url + '?userId=' + data[0].distinct_id;
      }else{
        url = sd.para.server_url + '&userId=' + data[0].distinct_id;
      }
      for(var i =0;i<data.length;i++){
        delete data[i].distinct_id;
      }
      data = JSON.stringify(data);
      data = encodeURIComponent(_.base64Encode(data));
      _.ajax({
        url: url,
        type: 'POST',
        cors: true,
        data: 'data=' + data,
        header: sd.para.custom_header
      });
      ...
      // };
      // if (sd.para.server_url.indexOf('?') !== -1) {
      //   this[state].src = sd.para.server_url + '&data=' + data;
      // } else {
      //   this[state].src = sd.para.server_url + '?data=' + data;
      // }
    ...
    saEvent.send = function(pArr, callback) {
      ...
      if (sd.para.debug_mode === true) {
        logger.info(dataAll);
        this.debugPath(JSON.stringify(dataAll), callback);
      } else {
        sd.sendState.getSendCall(dataAll, callback);
      }
    ...
    /*
     * 批量打包发送，接受数组形式时间内容 [{event:'',properties:{}},...]
     * @param {Array} pArr
     * @param {Function} c
     * */
    sd.trackInPack = function(pArr, c) {
      var dataPArr = [];
      for(var i=0;i<pArr.length;i++){
        var p = pArr[i];
        if (saEvent.check({event: p.event, properties: p.properties})) {
          dataPArr.push({
            type: 'track',
            event: p.event,
      ...
      if(dataPArr.length>0){
        saEvent.send(dataPArr, c);
      }
    };
